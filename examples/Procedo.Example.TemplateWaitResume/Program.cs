...
var workflowPath = args.Length > 0
    ...
    : Path.Combine(repoRoot, "examples", "61_template_wait_resume_release_pack_demo.yaml");

var stateDirectory = Path.Combine(repoRoot, ".procedo", "example-template-wait-resume");
...
workflow.Variables["handoff_dir"] = Path.Combine(stateDirectory, "handoff");
workflow.Variables["output_dir"] = Path.Combine(stateDirectory, "output");

const string runId = "template-wait-resume-example";
